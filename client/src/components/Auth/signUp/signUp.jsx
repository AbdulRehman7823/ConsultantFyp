...
function SignUp() {
  ...
  const [imgUrlDocument, setImgUrlDocument] = React.useState();
  ...
  const onDropDocument = (acceptedFiles, rejectedFiles, imgName) => {
    if (rejectedFiles.length > 0) {
      alert.showWarningAlert("Upload only one image and size limit of 1 MB");
      return;
    } else if (acceptedFiles) {
      convertImageToBase64(acceptedFiles[0], (result, success) => {
        if (success) {
          uploadImage(result, (url, success) => {
            if (success) {
              handleData("documentImage", `${url}`);
              setIurlDocument(`${url}`);
              setImgUrlDocument(acceptedFiles[0].name);
            }
          });
        }
      });
          ...
          >
            <div className="w-full bg-gray">
                ...
                <div className="flex flex-col justify-around">
                  <div className="flex items-center justify-center w-full">
                    {iurl == "" ? (
                      <label
                        for="dropzone-file"
                        className="px-12 py-4 flex flex-col items-center justify-center w-full h-44 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600"
                      >
                  ...
                  <div className="flex items-center justify-center w-full">
                    {iurlDocument == "" ? (
                      ...
                      >
                        ...
                        <FileUploader
                          placeholder={imgUrlDocument ? imgUrlDocument : "Click here to upload"}
                          accept={["image/jpeg", "image/png", "image/bmp"]}
                          maxFiles={1}
                          maxSize={1000000}
                          onDrop={(acceptedFiles, rejectedFiles) =>
              ...
              <p className="text-sm text-gray-500 ">
                &copy; 2022 Career - All Rights Reserved.
              </p>
            </div>
          </div>
        </section>
